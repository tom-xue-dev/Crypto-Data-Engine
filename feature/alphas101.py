...
class Alphas101:
    ...
    def alpha075(self):
        ...
        return (rank(correlation(self.vwap, self.volume, 4)) < rank(
            correlation(rank(self.low), rank(adv50), 12))).astype('int')
    ...
    def alpha095(self):
        ...
        return (rank((self.open - ts_min(self.open, 12))) * 12 < ts_rank(
            (rank(correlation(sma(((self.high + self.low) / 2), 19), sma(adv40, 19), 13)).pow(5)), 12)).astype('int')
...
if __name__ == '__main__':
    ...
    assets = select_assets(start_time=start, spot=True, n=3)
    # print(assets)
    ...
    for asset,group in data.groupby('asset'):
        group = group.reset_index(level = 'asset',drop=True)
        print(group)
        alphas = Alphas101(group)
        data['alpha001'] = alphas.alpha001()
        print(data)
