...
def calculate_alpha(data):
    ...
    """
    return (data['close'] - data['open']) / (data['high'] - data['low'] + 1e-6)
...
if __name__ == '__main__':
    ...
    while True:
        assets = select_assets(spot=True, n=3,start_time=start)
        print(assets)
    ...
    data['alpha'] = ans
    data['future_return'] = data['close'].rolling(75).mean() / data['close']
    data.dropna()
