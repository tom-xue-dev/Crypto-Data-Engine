...
def fetch_and_save(to_time, interval, file_name):
    """
    ...
    :param file_name: 输出CSV文件名
    """
    print(f"线程 {threading.current_thread().name} 获取数据，to: {utils.timestamp_to_datetime(to_time)}")
    data = _get_klines_data(to_time, interval)
    if not data:
        print(f"线程 {threading.current_thread().name} 没有获取到数据。")
        return
    _save_new_data(data, file_name)
    ...
    with ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
            ...
            while True:
                if mode == Mode.UPDATE and end_time - INTERVAL_TIME[interval] > CURRENT_TIME:
                    break
                # 提交任务到线程池
                futures.append(executor.submit(fetch_and_save, end_time, interval, file_name))
                
                # 更新end_time为下一个时间段的to_time
                ...
                # 控制请求频率
                sleep(random.uniform(0.5, 1.5))  # 随机延迟以模拟更自然的请求模式
